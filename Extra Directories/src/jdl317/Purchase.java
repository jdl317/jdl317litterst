/*
 * This class is the source code for the Purchase interface.  The class allows you to
 * buy a number of phones, create an account, edit an existing account, and 
...
public class Purchase {
    //Create data fields for Purchase object.  These include a scanner, a connection
    //to the database, an exit boolean, and a choice variable.
    ...
    //Essentially the home screen for the Purchase interface.
    private void beginPurchase() throws SQLException{
        ...
        if(online){
            ...
            System.out.println("\nThank you for choosing our store located at:");
            System.out.println("***************************************");
            try{
        ...
        //User must choose an option within the interface.
    ...
    //This method allows the user to purchase one or more cell phones from any JogWireless location
    private void buyPhone(int store) throws SQLException {
        //List<> variables are used to convert resultsets from SQL into usable variables in Java.
        ...
        boolean valid = false;
        int custID = -1;
        //These will be filled when user enters information.
        String first = "";
        String last = "";
        String street = "";
        String city = "";
        ...
        while(!exit){
            ...
            try{
                    ...
                    if(scan.hasNextInt()){
                        ...
                        if(phoneChoice < 201 || phoneChoice > 207){
                            if(phoneChoice == 0){
                                ...
                                System.out.println("Please select a valid phoneID.");
                            }
                        }
                        else{
                            //Valid phoneChoice
                ...
                //Find available quantity of selected phone, as well as its price
                s1 = con.createStatement();
            ...
            while(!valid){
                if(scan.hasNextInt()){
                    ...
                    if(custID < 0 || custID > 99999){
                        ...
                        try{
                            //Confirm that ID isn't already in database.
                            Statement s = con.createStatement();
            ...
            //Enter user information, for the database to store. Lengths of names, streets, etc. 
            //are based on their length in my SQL tables.
            System.out.println("\nFirst Name: ");
            valid = false;
            while(!valid){
                if(scan.hasNext()){
            ...
            while(!valid){
                if(scan.hasNextLine()){
                    ...
                    if(street.length() > 40){
                        System.out.println("This street name is too long. Please try again.");
                    }
                    else{
                        valid = true;
                        break;
            ...
            //Add appropriate infomation to customer, personal_customer, and sale tables.
            ...
            System.out.println("*************************************************************");
            //Option to repeat phone purchasing process.
    ...
    //This method allows the user to edit an existing account. 
    private void editAccount() throws SQLException{
        ...
        while(!exit){
            ...
            //Loop ensures that only an existing account IDs will be accepted.
            while(!valid){
                if(scan.hasNextInt()){
                    acctID = scan.nextInt();
            ...
            //Print current information for selected account
    ...
    //This method allows user to create account.
    private void createAccount() throws SQLException{
        ...
        while(!exit){
            ...
            valid = false;
            //Confirms that ID doesn't already exist.
                ...
                if(plan != 3){
                    ...
                    while(!valid){
                        if(scan.hasNextLine()){
                            city = scan.nextLine();
                            if(city.length() > 40){
                                System.out.println("This city name is too long. Please try again.");
                            }
                            else{
                                valid = true;
                                break;
            ...
            //Print account info that was just uploaded to database.
    ...
    //Allows user to restock the store(s) of their choice.
    private void restockStore(int store)throws SQLException{
        ...
        while(!exit){
            ...
            System.out.println("\nPerfect! Here is the updated inventory.");
            printStoreInv(store);
    ...
    //The next methods mostly deal with SQL.  As i progressed through the project,
    //I began creating more of these, as I tend to use them more than once. The buyPhone()
    //function is less efficient in that I wrote a lot of SQL right in the method, whereas 
    //restockStore() uses these more.  
    ...
    private void addCustomer(int acctID, String street, String city, String state)throws SQLException{
        try{
            Statement s = con.createStatement();
    ...
    private void addPersonalCustomer(int acctID, String last, String first)throws SQLException{
        try{
            Statement s = con.createStatement();
    ...
    private void addSale(int custID, double totalCost)throws SQLException{
        try{
            Statement s = con.createStatement();
    ...
    private void addBusinessCustomer(int acctID, String company)throws SQLException{
        try{
            Statement s = con.createStatement();
    ...
    private void addBillingPlan(int acctID, int payType, String moy)throws SQLException{
        double rate = 0;
        String payment = null;
        if(payType == 1){
            payment = "minute rate";
    ...
    private List<String> createNums(int numPhones, List<String> phone_nums){
    ...
    private void addPhoneNums(int acctID, int numPhones, List<String> phone_nums)throws SQLException{
        Random rand = new Random();
        for(int j = 0; j < numPhones; j++){
            try{
                Statement s = con.createStatement();
    ...
    private void addAccount(int acctID, String primary, int plan)throws SQLException{
    ...
    //Method used to print SQL columns in an efficient, easy way.
    ...
    private void printPlanInfo(String first, String last, String street, String city, String state, int acctID, int plan, List<String> phone_nums, int moy, int payType){
        String plan_type = null;
